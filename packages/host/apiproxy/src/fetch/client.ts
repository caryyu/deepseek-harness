...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected mintRpcId(): RpcId {
    // crypto.randomUUID is a Web API (browser + Node ≥19), but insecure origins
    // (plain-http LAN access) expose neither it nor crypto.subtle — mint a v4
    // UUID from crypto.getRandomValues, which insecure origins do expose.
