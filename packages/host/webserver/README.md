...
`gzipIfAccepted(acceptEncoding, body)` is the shared response-encoding helper for the file-serving consumers ([`dsh-client-modules`](../../client/modules/README.md) bundles and the frontend-static dist server): it gzip-compresses a fully-buffered body when the client's `Accept-Encoding` permits it (an explicit `gzip` token with `q` other than 0, or a bare `*` wildcard) and returns the identity body otherwise; callers send `Vary: accept-encoding` alongside so shared caches key on the choice. Streaming responses must not compress, so the helper exists for full-body responses only.
...
- **No TLS or origin policy** — binding a non-loopback address exposes the server to that network; deployment hardening (or fronting it with a real reverse proxy) is deliberately out of scope for the dev-facing v1. The auth gate adds access control, not confidentiality: over plain HTTP its credentials are sniffable, so a non-loopback deployment should still sit behind TLS.
...
- **gzip only** — the negotiated encoding is gzip; brotli stays deferred until a deployment needs it.
